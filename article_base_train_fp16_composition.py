...
def load_custom_dataset_from_csv(csv_file, image_folder):
    ...
    # Use only Composition category
    filtered_data = data[data['category'] == 'Composition']
    
    questions = filtered_data['question'].tolist()
    images = [os.path.join(image_folder, img) for img in filtered_data['image'].tolist()]
    answers = filtered_data['answer'].tolist()
...
# Check Whether data is filtered correctly
def check_filtered_data(dataset, num_samples=5):
    print(f"Number of filtered samples: {len(dataset)}")
    print("Showing a few samples:")
    for i in range(min(num_samples, len(dataset))):
        print(f"Sample {i+1}:")
        print(f"Question: {dataset[i]['question']}")
        print(f"Image path: {dataset[i]['image']}")
...
def main(args):
    ...
    # check_filtered_data(dataset, num_samples=5)
